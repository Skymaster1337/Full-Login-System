...
class User {
	...
	public static function updateUser($uid, $fname, $lname, $username, $email, $password) {
		$i = self::getInstance();
		//                     $uid, $fname, $lname, $username, $email
		return $i->update_user($uid, $fname, $lname, $username, $email);
	}
	...
	/**
	 * Update the User Account, Can be used by Admins to update Accounts.
	 * @see reg_user
	...
	public function update_profile($uid, $fname, $lname, $email, $address, $zipcode, $city, $phone){
		...
		// If the Username & the Email are not used already then register the account.
		if($count_row !== 0){
			$query = "UPDATE  `users` SET  `fname` = '$fname', `lname` = '$lname', `uemail` = '$email', `address` = '$address', `zipcode` = '$zipcode', `city` = '$city', `phone` = '$phone' WHERE  `uid` ='$uid'";
			$result = $this->db->query($query) or die($this->db->error);
			return true;
		} else {
			return false;
	...
	/**
	 * Update User Password, If the input is blank, Discarded Update.
	 *  This Method Uses the Current User, Not Selected Input.
	 */
	public function update_password_cu($password = ""){
...
function hash_psw($in = ""){
    if(defined('AUTH_SALT')){
        return hash('sha256', AUTH_SALT.$in);
        //return password_hash(AUTH_SALT.$in, PASSWORD_BCRYPT);
    }
    else {
